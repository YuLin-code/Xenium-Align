...
def plot_voronoi_polygons(voronoi_polygons, voronoi_polygon_colors=None):
    ...
    #print('voronoi_polygons',voronoi_polygons)
    ...
    for polygon, polygon_color in zip(voronoi_polygons, voronoi_polygon_colors):
        ...
        #print('polygon_color',polygon_color)
...
def build_graph_from_cell_coords(cell_data, voronoi_polygons):
    ...
    if not len(cell_data) == len(voronoi_polygons):
        ...
        save_polygon = False
    #print('build_graph_from_cell_coords len(voronoi_polygons)',len(voronoi_polygons))
    #print('build_graph_from_cell_coords len(cell_data)',len(cell_data))
    #print('build_graph_from_cell_coords cell_data',cell_data)
    
    coord_ar = np.array(cell_data[['CELL_ID', 'X', 'Y']])
    #print('build_graph_from_cell_coords coord_ar',coord_ar)
    G = nx.Graph()
    node_to_cell_mapping = {}
    for i, row in enumerate(coord_ar):
        vp = voronoi_polygons[i] if save_polygon else None
        #print('build_graph_from_cell_coords vp',vp)
    ...
    #print('build_graph_from_cell_coords G',G)
    #print('build_graph_from_cell_coords list(G.edges)',list(G.edges))
    #print('build_graph_from_cell_coords node_to_cell_mapping',node_to_cell_mapping)
...
def construct_graph_for_region(region_id,
    ...
    #print('G.nodes[0][voronoi_polygon]',G.nodes[0]['voronoi_polygon'])
    #print('G.nodes[0]',G.nodes[0])
    #print('list(G.edges)',list(G.edges))
    
    G.region_id = region_id

    ## Visualization of cellular graph                                         #orignal code
    #if voronoi_polygon_img_output is not None:                                #orignal code
    #    plt.clf()                                                             #orignal code
    #    plt.figure(figsize=(figsize, figsize))                                #orignal code
    #    plot_voronoi_polygons(G)                                              #orignal code
    #    plt.axis('scaled')                                                    #orignal code
    #    plt.savefig(voronoi_polygon_img_output, dpi=300, bbox_inches='tight') #orignal code
    #if graph_img_output is not None:                                          #orignal code
    #    plt.clf()                                                             #orignal code
    #    plt.figure(figsize=(figsize, figsize))                                #orignal code
    #    plot_graph(G)                                                         #orignal code
    #    plt.axis('scaled')                                                    #orignal code
    #    plt.savefig(graph_img_output, dpi=300, bbox_inches='tight')           #orignal code

    # Visualization of cellular graph                                         
    if voronoi_polygon_img_output is not None and G.nodes[0]['voronoi_polygon'] is not None:
        plt.clf()                                                             
